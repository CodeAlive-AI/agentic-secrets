...
AgenticFortress does not generate per-CLI shell shims for registered apps in the default self-build flow. Installed AgenticFortress executables are symlinked under `$PREFIX/bin`, while registered apps are stored as metadata in `cli-registry.json`. When a CLI is auto-discovered from `PATH`, the registry keeps the stable invocation path such as `/opt/homebrew/bin/hcloud`; each `cli run` resolves and hashes the current target binary before execution. Normal Homebrew upgrades should keep working because the stable symlink moves to the new Cellar version. If you manually register a versioned path such as `/opt/homebrew/Cellar/hcloud/1.65.0/bin/hcloud`, that registration is intentionally pinned and must be re-registered after the version is removed.
