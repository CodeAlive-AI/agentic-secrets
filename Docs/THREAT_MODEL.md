...
- Repeated CLI reads may reuse a short HMAC-signed unlock grant scoped to CLI name, target identity, workspace hash, action class, parent app, delivery mode, and secret alias. Grants contain no secret material and expire quickly.
...
| CLI unlock grants | Elevation of Privilege | Unlock grants are HMAC-signed, short-lived, scope-bound, and store no secret material. Tampering with expiry or scope invalidates the grant; non-matching target/workspace/action/secret prompts again. | `CLIUnlockGrantStore` contract tests. Owner: core runtime. |
