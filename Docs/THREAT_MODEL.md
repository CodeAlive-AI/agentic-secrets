...
- API sessions and MCP paths pin profiles and block cross-origin redirects by default.
...
- Agentic Secrets does not make approved target binaries trustworthy.
- Agentic Secrets does not defend against root or kernel compromise.
- AgenticSecrets only partially reduces risk from same-user malware.
...
- `agentic-secrets-brokerd`: policy, approvals, invocation handles, audit, rollback decisions.
- `agentic-secrets-shim`: symlink-invoked delivery sink and exec planner.
- `agentic-secrets-bitwarden-providerd`: Bitwarden token owner and one-secret provider helper.
- `agentic-secrets-api-sessiond`: profile-pinned local API proxy.
- `agentic-secrets-mcpd`: stdio to Streamable HTTP MCP credential bridge.
- Secret Broker-owned local encrypted secret store and LocalAuthentication for production self-build secret unlock and user presence. Optional future Keychain-backed storage is tracked separately for provisioned signing.
- Signed command policy packs that classify commands.
...
- API session runtime: local token, method/path allowlist, upstream origin pinning, no body logging.
...
| --- | --- | --- | --- |
| Helper to core IPC | Spoofing | Self-build helper identity is authenticated by install manifest, resolved path, owner, permissions, minimum version, binary SHA-256, and optional cdhash. Developer ID Team ID is optional future evidence, not required for local source builds. | `SelfBuildPeerValidator`, `BrokerIPCAuthorizer`, Unix socket contract tests. Owner: broker runtime. |
| Helper to core IPC | Tampering | IPC messages are Codable, versioned, length-prefixed, and rejected on malformed payloads or unsupported protocol versions. | `BrokerIPCCodec`, `UnixDomainSocketIPCClient`, `UnixDomainSocketIPCServer`, contract tests. Owner: broker runtime. |
| Raw protocol parsing | Tampering | Complex parsers for untrusted adapter/MCP/provider/raw byte streams are isolated from Secret Broker secret authority. Secret Broker accepts only bounded, typed results and fails closed on parser errors. | Parser-boundary acceptance criteria, fixed malformed fixtures, and architecture review before adding new raw parsers. Owner: parser subsystem. |
...
| Adapter registry | Tampering | External command policy packs are signed declarative data. Registration enforces trusted key id, publisher allowlist, CLI allowlist, schema version, expiry, rule validation, golden fixtures, and rollback checks. | `CommandPolicyPackVerifier`, `PolicyPackRegistry`, contract tests. Owner: adapter subsystem. |
| Bitwarden provider path | Information Disclosure | Runtime fetch is scoped to exactly one approved alias and sink. List and project-wide runtime operations are unavailable. | `BitwardenProviderPolicy`, `BitwardenProviderRuntime`, contract tests. Owner: provider subsystem. |
| Bitwarden provider path | Denial of Service | Provider leases are short-lived by environment and prod requires per-fetch approval. Rotation has an ordered workflow that includes lease invalidation. | `BitwardenProviderLeasePolicy`, `BitwardenRotationWorkflow`, contract tests. Owner: provider subsystem. |
| Local API session | Spoofing | API session requests require a localhost session capability token bound to profile, port, and TTL. | `APISessionAuthorizer` tests. Owner: API session subsystem. |
| Local API session | Tampering | Upstream origin, method, and path prefixes are pinned; cross-origin redirects are denied. | `APISessionRuntime`, redirect tests. Owner: API session subsystem. |
| Local API session | Information Disclosure | Request and response bodies are not logged by default; authorization metadata is redacted. | `APISessionRuntime.bodyForAudit`, audit metadata tests. Owner: API session subsystem. |
