...
| Keychain secret resolution | Information Disclosure | Only core-owned code may use `KeychainSecretStore`; helper and CLI targets are statically blocked from production Keychain read paths. Keychain items use data-protection Keychain, device-local accessibility, and user-presence access control. | `scripts/check_secret_authority.sh`, `KeychainSecretQueryFactory` tests. Owner: core runtime. |
