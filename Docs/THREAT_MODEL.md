...
| CLI unlock grants | Elevation of Privilege | Unlock grants are HMAC-signed, short-lived, scope-bound, and store no secret material. Tampering with expiry or scope invalidates the grant; non-matching target/workspace/secret prompts again, while action-level command policy is re-evaluated for every invocation. | `CLIUnlockGrantStore` contract tests. Owner: core runtime. |
