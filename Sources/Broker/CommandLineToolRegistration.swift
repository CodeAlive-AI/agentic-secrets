...
public struct EnvironmentSecretBinding: Codable, Equatable, Sendable {
...
public struct CommandLineToolRegistration: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        environmentBindings: [EnvironmentSecretBinding],
        registeredAt: Date
    ) {
        self.name = name
...
public struct CommandLineToolRegistrationDocument: Codable, Equatable, Sendable {
    public var schemaVersion: Int
    public var registrations: [String: CommandLineToolRegistration]

    public init(schemaVersion: Int = 1, registrations: [String: CommandLineToolRegistration] = [:]) {
        self.schemaVersion = schemaVersion
        self.registrations = registrations
    }
}

public struct CommandLineToolTrustRefreshAuthorizationRequest: Codable, Equatable, Sendable {
    public var name: String
    public var targetPath: String
...
public enum CommandLineToolRegistrationError: Error, Equatable, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .targetIdentityChanged(let name, let expected, let actual):
            "Registered target identity changed for '\(name)'. Expected \(expected), got \(actual). Verify the target binary, then run `agentic-secrets cli trust-refresh \(name)`."
...
public struct CommandLineToolStore: Sendable {
    public var registryURL: URL
    public var integrityProtector: (any ToolRegistryIntegrityProtector)?

    public init(registryURL: URL, integrityProtector: (any ToolRegistryIntegrityProtector)? = nil) {
    ...
    public func load() throws -> CommandLineToolRegistrationDocument {
        guard FileManager.default.fileExists(atPath: registryURL.path) else {
            return CommandLineToolRegistrationDocument()
        }
        let registryData = try Data(contentsOf: registryURL)
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        let document = try decoder.decode(CommandLineToolRegistrationDocument.self, from: registryData)
        guard document.schemaVersion == 1 else {
            throw CommandLineToolRegistrationError.unsupportedSchema(document.schemaVersion)
        }
        if let integrityProtector {
            guard FileManager.default.fileExists(atPath: integrityURL.path) else {
                ...
                throw ToolRegistryIntegrityError.missingSignature(integrityURL.path)
            }
            let integrity = try decoder.decode(ToolRegistryIntegrity.self, from: Data(contentsOf: integrityURL))
            try integrityProtector.verify(registryData, integrity: integrity)
        }
        return document
    }

    public func save(_ document: CommandLineToolRegistrationDocument) throws {
...
public struct CommandLineToolRegistry: Sendable {
    public var registryStore: CommandLineToolStore
    public var secretStore: LocalEncryptedSecretStore

    public init(registryStore: CommandLineToolStore, secretStore: LocalEncryptedSecretStore) {
    ...
    public func register(
        ...
        now: Date = Date()
    ) throws -> CommandLineToolRegistration {
        ...
        guard !environmentValues.isEmpty else {
            throw CommandLineToolRegistrationError.missingEnvironmentValue("*")
        }
        ...
        let bindings = try environmentValues.keys.sorted().map { environmentName in
            ...
            guard let material = environmentValues[environmentName] else {
                throw CommandLineToolRegistrationError.missingEnvironmentValue(environmentName)
            }
            try secretStore.store(alias: SecretAlias(alias), material: material, label: "\(name) \(environmentName)", environment: "cli:\(name)")
            return EnvironmentSecretBinding(environmentName: environmentName, secretAlias: alias)
        }

        var document = try registryStore.load()
        let registration = CommandLineToolRegistration(
            name: name,
    ...
    public func unregister(name: String, deleteSecrets: Bool = false) throws -> CommandLineToolRegistration {
        var document = try registryStore.load()
    ...
    public func registration(named name: String) throws -> CommandLineToolRegistration {
    ...
    public func refreshTargetTrust(
        name: String,
        authorize: (CommandLineToolTrustRefreshAuthorizationRequest) throws -> Void
    ) throws -> CommandLineToolRegistration {
        var document = try registryStore.load()
        ...
        try authorize(CommandLineToolTrustRefreshAuthorizationRequest(
        ...
        guard trustFingerprint(proposed) == trustFingerprint(confirmed) else {
            throw CommandLineToolRegistrationError.targetChangedDuringTrustRefresh(name: name)
        }
    ...
    private func refreshedRegistration(from registration: CommandLineToolRegistration) throws -> CommandLineToolRegistration {
    ...
    private func trustFingerprint(_ registration: CommandLineToolRegistration) -> String {
        [
    ...
    public func validateTargetIdentity(registration: CommandLineToolRegistration, assessedTarget: TargetAssessment) throws {
        if let requirement = registration.targetDesignatedRequirement {
            guard CodeSignatureInspector.satisfies(path: assessedTarget.resolvedPath, requirementText: requirement) else {
                throw CommandLineToolRegistrationError.targetIdentityChanged(name: registration.name, expected: requirement, actual: assessedTarget.identity)
            }
            return
        }
        ...
        guard expectedIdentity == assessedTarget.identity else {
            throw CommandLineToolRegistrationError.targetIdentityChanged(name: registration.name, expected: expectedIdentity, actual: assessedTarget.identity)
        }
    }
    ...
    private func validateCLIName(_ name: String) throws {
        guard name.range(of: #"^[A-Za-z0-9][A-Za-z0-9._-]*$"#, options: .regularExpression) != nil,
              !name.contains("/") else {
            throw CommandLineToolRegistrationError.invalidCLIName(name)
        }
    }
    ...
    private func validateEnvironmentName(_ name: String) throws {
        guard name.range(of: #"^[A-Za-z_][A-Za-z0-9_]*$"#, options: .regularExpression) != nil else {
            throw CommandLineToolRegistrationError.invalidEnvironmentName(name)
        }
    }
    ...
    private func validateExecutable(_ path: String) throws {
        ...
        guard FileManager.default.fileExists(atPath: resolved, isDirectory: &isDirectory),
            ...
            throw CommandLineToolRegistrationError.targetNotExecutable(path)
        }
    }
...
public struct LocalInstallLayout: Sendable {
    ...
    public var deliveryGrantsURL: URL {
        stateDirectory.appendingPathComponent("delivery-grants.json")
    }

    public var deliveryGrantKeyURL: URL {
        stateDirectory.appendingPathComponent("secrets/delivery-grants.key")
    }

    public var rememberedApprovalsURL: URL {
        stateDirectory.appendingPathComponent("cli-persistent-allow-grants.json")
    }

    public var cliPersistentAllowIntegrityProtector: any ToolRegistryIntegrityProtector {
        KeychainToolRegistryIntegrityProtector(
            service: "com.agenticsecrets.cli-persistent-allow",
            account: "local-state:" + shortDigest(stateDirectory.standardizedFileURL.path, length: 24)
        )
    }

    public var configURL: URL {
        stateDirectory.appendingPathComponent("config/agentic-secrets.json")
    }

    public var registrationService: CommandLineToolRegistry {
        CommandLineToolRegistry(
            registryStore: CommandLineToolStore(
                registryURL: registryURL,
                integrityProtector: KeychainToolRegistryIntegrityProtector(
                    account: "local-state:" + shortDigest(stateDirectory.standardizedFileURL.path, length: 24)
                )
            ),
    ...
    public static func defaultStateDirectory() -> URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/AgenticSecrets/LocalState", isDirectory: true)
    }
}
