...
public enum DeliveryChannel: String, Codable, CaseIterable, Sendable {
    case cliEnv = "cli-env"
    case apiSession = "api-proxy"
    case bitwardenProvider = "bitwarden-provider"
    case remoteMCP = "remote-mcp"
    case remoteSSHStdin = "remote-ssh-stdin"
    case cloudNativeIdentity = "cloud-native-identity"
}

public enum DeliveryMechanism: String, Codable, Sendable {
    case env
    case proxy
    case stdin
...
public struct DeliveryContract: Codable, Equatable, Sendable {
    public var profile: DeliveryChannel
    public var claim: String
    public var nonClaims: [String]
    public var tcb: [String]

    public init(profile: DeliveryChannel, claim: String, nonClaims: [String], tcb: [String]) {
...
public enum DeliveryContracts {
    public static let all: [DeliveryContract] = [
        DeliveryContract(
            profile: .cliEnv,
            ...
            tcb: ["agentic-secrets-shim", "agentic-secrets-brokerd", "broker-owned local secret store", "approved target CLI"]
        ),
        DeliveryContract(
            profile: .apiSession,
            claim: "Development app receives only a local proxy endpoint and per-session proxy token; the real upstream API key stays in agentic-secrets-api-sessiond/provider path.",
            nonClaims: [
                "A localhost proxy token is still a local bearer capability.",
                "Does not hide API use from all same-user local attackers."
            ],
            tcb: ["agentic-secrets-api-sessiond", "agentic-secrets-brokerd", "provider helper", "upstream API"]
        ),
        DeliveryContract(
            profile: .bitwardenProvider,
            claim: "Bitwarden access token is owned by agentic-secrets-bitwarden-providerd and is not placed in env, argv, files, logs, MCP configs, or core plaintext state.",
            nonClaims: [
                "Bitwarden machine account blast radius remains equal to its upstream scope.",
                "A compromised provider helper can use approved scope during an active lease."
            ],
            tcb: ["agentic-secrets-bitwarden-providerd", "BWS SDK/CLI", "broker-owned local secret store", "agentic-secrets-brokerd authorization"]
        ),
        DeliveryContract(
            profile: .remoteMCP,
            claim: "Bearer token is not stored in MCP client config and is injected only by agentic-secrets-mcpd into requests for a pinned upstream profile.",
            nonClaims: [
                "Does not prove remote MCP server safety.",
                "Tool filtering is a guardrail, not a security boundary."
            ],
            tcb: ["agentic-secrets-mcpd", "agentic-secrets-brokerd", "provider helper", "pinned MCP upstream"]
        )
    ]
}

public enum SecurityInvariant: String, CaseIterable, Codable, Sendable {
    ...
    case bitwardenSingleSecretRuntime = "I8.bitwardenProviderBindings-single-secret-runtime"
