...
public struct ProcessOriginHint: Codable, Equatable, Sendable {
    ...
    public static func current(
        ...
        if let firstExternal = chain.first(where: { !isAgenticSecretsProcess($0) && !isShellProcess($0) }) {
    ...
    private static func isAgenticSecretsProcess(_ name: String) -> Bool {
        name == "AgenticSecrets" || name.hasPrefix("agentic-secrets")
    }
...
public struct InstallManifest: Codable, Equatable, Sendable {
    ...
    public init(
        schemaVersion: Int = 1,
        productName: String = "AgenticSecrets",
        appVersion: String,
        prefix: String,
        installedAt: Date,
...
public enum LocalAuthenticationGate {
    public static func validate(proof: LocalAuthenticationProof, manifest: DeliveryDecisionManifest, maxAge: TimeInterval = 30, now: Date = Date()) throws {
    ...
    public static func reason(for manifest: DeliveryDecisionManifest) -> String {
    ...
    private static func readableCommand(from manifest: DeliveryDecisionManifest) -> String {
