...
public struct LeaseScope: Codable, Equatable, Sendable {
    ...
    public init(subject: String, adapterIdentity: String = "builtin-legacy", secretAlias: String, workspaceHash: String, originHint: String, actionClass: String, configContext: String, deliveryMode: DeliveryMechanism, targetIdentity: String) {
...
public struct PolicyEngine: Sendable {
    ...
    public func authorize(command: NormalizedCommand, intent: DeliveryRequest, target: TargetAssessment, approval: ApprovalOption, state: PolicyState, now: Date = Date()) throws -> PolicyDecision {
        ...
        case .providerLease5m:
            guard command.risk != .destructive else { throw PolicyError.destructiveRememberDenied }
            let scope = LeaseScope(subject: command.cli, adapterIdentity: command.adapterIdentity?.leaseComponent ?? "missing-adapter-identity", secretAlias: intent.secretAlias, workspaceHash: "hmac:" + shortDigest(intent.workspace, length: 16), originHint: intent.originHint, actionClass: command.actionClass, configContext: "provider;\(DeliveryDecisionManifestFactory.configContext(for: command))", deliveryMode: intent.delivery, targetIdentity: target.identity)
