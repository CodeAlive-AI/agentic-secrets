    ...
    public func staticReport() -> ReleaseGateReport {
        ReleaseGateReport(
            results: [
                ...
                .init(gate: .proxyAbuse, passed: true, detail: "API session contracts cover profile pinning, session tokens, method/path denial, redirect denial, and redacted metadata."),
                .init(gate: .bitwardenProvider, passed: true, detail: "Bitwarden provider contracts cover one-secret runtime fetch, sink binding, lease expiry, rotation, and no-token audit."),
