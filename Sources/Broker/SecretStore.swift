...
public struct KeychainSecretStore: LocalSecretStore {
    ...
    public init(service: String = "com.agenticsecrets.secrets", backend: KeychainStorageBackend = .loginKeychain, descriptors: [KeychainSecretDescriptor] = []) {
        self.service = service
        self.backend = backend
...
public final class ApprovalSessionStore: @unchecked Sendable {
    ...
    public func create(manifest: DeliveryDecisionManifest, policyEpoch: Int, ttl: TimeInterval, now: Date = Date()) -> ApprovalSession {
    ...
    public func validate(sessionID: String, manifest: DeliveryDecisionManifest, policyEpoch: Int, now: Date = Date()) throws -> ApprovalSession {
        try lock.withLock {
