...
public enum RemoteDeliveryPattern: String, Codable, Sendable {
    ...
    case remoteBWSMachineAccount = "remote-bws-machine-account"
    case cloudNativeIdentity = "cloud-native-identity"
    case remoteEnvFile = "remote-env-file"
}

public struct RemoteDeliveryClaim: Codable, Equatable, Sendable {
...
public enum RemoteDeliveryCatalog {
    public static let all: [RemoteDeliveryClaim] = [
        ...
        .init(pattern: .remoteBWSMachineAccount, claim: "Local Mac does not ship application secrets; server uses its own scoped machine identity.", nonClaim: "Machine account blast radius is defined by upstream BWS scope.", requiresWarning: false),
        .init(pattern: .cloudNativeIdentity, claim: "Workload receives cloud-native short-lived identity instead of a copied static secret.", nonClaim: "Cloud IAM policy quality is outside AgenticFortress's local control.", requiresWarning: false),
